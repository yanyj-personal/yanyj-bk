...
let forecastTask = async (periods = '') => {
   getPage('http://odds.zgzcw.com/macao/sfc/' + periods).then(async res => {
    ...
    // 期数
    let periodS = $('#xchg').find("option:selected").val();
    ...
    $('.xs_trbg').map(function (tag) {
	  ...
	  if(socre) {
		  ...
		  forecast.hostFinalNumber = +socre[0].split(':')[0] ?  +socre[0].split(':')[0]: 0;
		  forecast.awayFinalNumber = +socre[0].split(':')[1]?  +socre[0].split(':')[1]: 0;
		  forecast.hostHalfNumber = +socre[1].split(':')[0]?  +socre[1].split(':')[0]: 0;
		  forecast.awayHalfNumber = parseInt(socre[1].split(':')[1]) ? parseInt(socre[1].split(':')[1]): 0;
